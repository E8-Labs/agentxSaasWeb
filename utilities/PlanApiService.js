...
export class PlanApiService {
  ...
  static async getAgencyPlans() {
    try {
      const response = await fetch(`${BASE_URL}/api/agency/getPlanListForAgency`, {
        method: 'GET',
  ...
  static async updateAgencyPlan(planId, planData) {
    try {
      const response = await fetch(`${BASE_URL}/api/admin/agency-plans/${planId}`, {
        method: 'PUT',
        headers: AuthHelper.getAuthHeaders(),
        body: JSON.stringify(planData),
      });
  ...
  static async updateAgentXPlan(planId, planData) {
    try {
      const response = await fetch(`${BASE_URL}/api/admin/agentx-plans/${planId}`, {
        method: 'PUT',
        headers: AuthHelper.getAuthHeaders(),
        body: JSON.stringify(planData),
      });
  ...
  static async getPlan(planType, planId) {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/admin/${endpoint}/${planId}`, {
        method: 'GET',
        headers: AuthHelper.getAuthHeaders(),
      });
