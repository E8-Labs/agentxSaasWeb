...
class PlansService {
   ...
   */
  static async getPlans(category = 'regular', context = 'default', includeTrial = false, environment = 'test') {
    try {
      ...
      const response = await axios.get(`${Apis.getPlans}?${params.toString()}`, {
        headers: {
  ...
  static getFallbackPlans(context, includeTrial = false) {
    const basePlans = [
      {
        id: 1, mints: 30, calls: 125,
        details: "Great for trying out AI sales agents.",
        originalPrice: "", discountPrice: "45", planStatus: "", status: ""
      },
      {
        id: 2, mints: 120, calls: "500",
        details: "Perfect for lead updates and engagement.",
        originalPrice: "165", discountPrice: "99", planStatus: "40%", status: ""
      },
      {
        id: 3, mints: 360, calls: "1500",
        details: "Perfect for lead reactivation and prospecting.",
        originalPrice: "540", discountPrice: "299", planStatus: "50%", status: "Popular"
      },
      {
        id: 4, mints: 720, calls: "5k",
        details: "Ideal for teams and reaching new GCI goals.",
        originalPrice: "1200", discountPrice: "599", planStatus: "50%", status: "Best Value"
      }
    ];
    ...
    if (context === 'onboarding' && includeTrial) {
      return [
        {
          id: 1, startFreeLabel: "Free", mints: 30, calls: 125, isTrial: true,
          trial: "7 Day Trial", details: "Perfect to start for free, then $45 to continue.",
          originalPrice: "45", discountPrice: "Free Trial", planStatus: "Free", status: ""
        },
        ...basePlans.slice(1)
      ];
    }
   ...
   */
  static async getCachedPlans(cacheKey, category, context, includeTrial = false) {
    try {
