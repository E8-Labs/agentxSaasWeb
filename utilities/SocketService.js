...
class SocketService {
  ...
  setupEventListeners() {
    ...
    this.socket.on('profile-updated', (profileData) => {
      ...
      if (profileData?.data?.id && userData?.userId && profileData.data.id === userData.userId) {
        ...
        console.warn('Profile user ID:', profileData?.data?.id, 'Current user ID:', userData?.userId);
      }
    });
  }
