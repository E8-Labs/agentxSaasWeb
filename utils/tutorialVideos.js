...
const getDefaultTutorial = (videoType) => {
  ...
  return defaultTutorials[videoType] || {
    ...
    if (user?.userRole === "AgencySubAccount") {
      ...
      return defaultTutorials.map(defaultTutorial => {
        ...
        if (agencyVideo) {
          ...
          const formattedDuration = agencyVideo.videoDuration && agencyVideo.videoDuration > 0
            ? formatDuration(agencyVideo.videoDuration)
            : defaultTutorial.description;
          
          console.log(`[TutorialVideos] Merging agency video for ${defaultTutorial.videoType}:`, {
