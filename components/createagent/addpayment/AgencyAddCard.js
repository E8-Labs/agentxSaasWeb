...
import SignupHeaderMobile from '@/components/onboarding/mobileUI/SignupHeaderMobile'
import { Checkbox } from '@/components/ui/checkbox'
import { getPolicyUrls } from '@/utils/getPolicyUrls'
import { getCardImage, getCardsList } from '@/components/plan/PlansUtilities'
...
const AgencyAddCard = ({
  ...
  const fetchCardsList = async () => {
    ...
    const cards = await getCardsList(selectedUser)
    if (cards) {
      setCards(cards);
      setShowAddCardForm(false)
      // setSelectedCard(cards[0])
    } else {
      toast.error('Error fetching cards list in parent modal')
    }
  ...
  const handleAddCard = async (e) => {
    ...
    if (result.error) {
      ...
      if (result2.status) {
        setAddCardSuccess(true)
        if (!togglePlan) {
          console.log('no plan to subscribe, just close', result)
          // fetchCardsList()
  ...
  const handleSubscribePlan = async () => {
    ...
    console.log("trigering handleSubscribePlan")
          ...
          >
            ...
            {/* Card section 
            {
              getCardsLoader ? (
                ...
                <div>
                  <div style={{ fontWeight: '600', fontSize: 16, color: 'black', marginTop: '10px' }}>
                    Select a card to continue
                  </div>
                  {
                    cards?.length > 0 ? (
                      <div className="w-full max-h-[20vh] pb-4 overflow-y-auto">
                        {
                          cards?.map((item) => (
            ...
            >
              <div className="flex w-full flex-row items-center justify-between">
                ...
                </div>
                {
                  showAddCardForm && (
                    <button
                      className="text-brand-primary underline font-semibold text-lg mt-4"
                        ...
                        <div>
                          <div className="w-full flex flex-row items-center justify-between mt-4">
                            ...
                            <button
                              ...
                              className="text-base font-medium text-brand-primary hover:text-brand-primary/80"
                            >
                              + Add Payment
                            </button>
                          </div>
                          {
                            cards?.length > 0 ? (
                              <div className="w-full h-[35vh] pb-4 overflow-y-auto">
                                {
                                  cards?.map((item) => (
                                    <div className="w-full mt-4" key={item.id}>
                                      <button
