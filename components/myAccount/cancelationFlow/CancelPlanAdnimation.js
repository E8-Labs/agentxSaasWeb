...
export default function CancelPlanAnimation({
  ...
  const isAgencySubAccount =
    isSubaccount || userLocalData?.userRole === 'AgencySubAccount'
  ...
  const getCUrrentComponent = async () => {
    ...
    if (
      (isPaused || userLocalData?.subscriptionPauseUsed > 0) &&
      !isAgencySubAccount
    ) {
      if (
            ...
            {currentIndex === 5 && (
              ...
              >
                <div className="">
                  ...
                  <CancelationCompleted handleContinue={handleContinue} />
                </div>
              </motion.div>
            )}
          </AnimatePresence>
