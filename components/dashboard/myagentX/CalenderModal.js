...
function CalendarModal(props) {
  ...
  const [showAddNewGoogleCalender, setShowAddNewGoogleCalender] =
    useState(false)
  ...
  const [googleConnectCalendarLoader, setGoogleConnectCalendarLoader] =
    useState(false)
  ...
  const selectCalendarView = () => {
    return (
      <div className="flex flex-col w-full items-center bg-white rounded-lg">
        ...
        <h2 className="text-lg font-semibold mb-4">Select a Calendar</h2>
              ...
              <FormControl sx={{}} className="w-full h-[50px]">
                ...
                >
                  {timeZones.map((item, index) => {
                    return (
                      <MenuItem className="w-full" value={item} key={index}>
  ...
  const renderView = () => {
    if (showAddNewCalender) {
      return calCalendarView()
    } else if (showAddNewGoogleCalender) {
      return googleCalView()
    } else if (showAddNewGHLCalender) {
      return GHLCalendarView()
    } else {
  ...
  function isEnabled(google = false) {
    ...
    } else if (calenderTitle && calenderApiKey && eventId && selectTimeZone) {
      // //console.log;
      return true
    } else {
      // //console.log;
