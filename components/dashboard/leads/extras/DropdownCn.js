      ...
      <DropdownMenu open={open} onOpenChange={setOpen}>
        <div className="flex items-center rounded-md border border-muted/0.9 bg-white shadow-sm">
          <DropdownMenuTrigger asChild>
            <button className={cn("flex items-center px-4 py-[1px] text-base font-regular focus:outline-none rounded-l-md", backgroundClassName)}>
          ...
          {onChevronClick ? (
            ...
            <div className="flex items-center justify-center px-2 py-[1px]">
              <IconComponent className={cn("h-4 w-4 text-foreground", backgroundClassName?.includes('text-white') && 'text-white')} />
            </div>
          )}
        </div>
