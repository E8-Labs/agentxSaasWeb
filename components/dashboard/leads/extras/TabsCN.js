...
const TabsCN = ({ tabs, value, onValueChange, className }) => {
  ...
  return (
    <Tabs value={value} onValueChange={onValueChange} className={cn('w-full', className)}>
      <TabsList className="w-full flex flex-row items-stretch gap-3 bg-transparent p-0 h-auto rounded-none border-b border-border/40">
        {tabs.map((tab) => {
          ...
          return (
            <TabsTrigger
              ...
              value={tab.id}
              className={cn(
                'flex flex-1 flex-row items-center justify-center gap-2 px-3 py-2 rounded-none border-b-2 border-transparent bg-transparent shadow-none',
                'border-0 border-b-2',
