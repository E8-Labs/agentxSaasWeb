...
const DynamicDropdown = () => {
    ...
    const handlePromptTagSelection = (tag) => {
        const beforeCursor = scriptTagInput.slice(0, textFieldRef.current.selectionStart);
        const afterCursor = scriptTagInput.slice(textFieldRef.current.selectionStart);
                ...
                >
                    {(promptDropDownVisible ? tags : kycsData).map((item) => (
                        <Typography
                            key={item.id || item}
                            onClick={() =>
                                handlePromptTagSelection(promptDropDownVisible ? item : item.question)
                            }
                            sx={{
                                padding: '8px 12px',
                                cursor: 'pointer',
