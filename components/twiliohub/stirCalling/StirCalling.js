        ...
        if (response) {
          ...
          if (apiResponse.status === true) {
            ...
            setShowSnack({
              type: SnackbarTypes.Error,
              message:
                apiResponse.message || 'Failed to create STIR/SHAKEN product',
              isVisible: true,
            })
          }
    ...
    >
      ...
      >
        <div className="max-h-[80svh]  w-full flex flex-col items-center justify-between">
          ...
          />
          <div className="w-full max-h-[80%] overflow-x-hidden overflow-y-auto scrollbar scrollbar-track-transparent scrollbar-thin scrollbar-thumb-purple pb-2 px-2">
            ...
            <div className="mt-2" style={{ fontWeight: '700', fontSize: 17 }}>
              {`Enter a display name for SHAKEN/STIR`}
            </div>
            <div className="mt-2" style={styles.normalTxt}>
              We will enable SHAKEN/STIR for outbound calls on all United States
              numbers assigned to this Twilio Approved Business Profile. No
              additional configuration is required. Enabling SHAKEN/STIR Trusted
              calling will not interupt your existing services
            </div>
