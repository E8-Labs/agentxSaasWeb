...
const AddScoringModal = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (!event.target.closest('.question-input-container')) {
        setQuestions((prev) =>
          prev.map((q) => ({ ...q, showSuggestions: false })),
        )
      }
    }
  ...
  const getFilteredSuggestions = (currentQuestion) => {
    ...
    const filteredQuestions = agentQuestions.filter(
      (q) => !q.trim().startsWith('//'),
    )
  ...
  const isValidForm = () => {
    ...
    const validQuestions = questions.filter(
      (q) => q.question.trim().length > 0 && parseFloat(q.points) > 0,
    )
      ...
      >
        <div className="w-full flex flex-col h-full">
              ...
              <FormControl sx={{ m: 1 }} className="w-[40%]">
                <Select
                  ...
                  renderValue={(selected) => {
                    ...
                    const template = templates.find(
                      (t) => t.id === selected || t.id === parseInt(selected),
                    )
          ...
          >
            ...
            <div
              className="w-full flex-row flex items-center gap-2 mt-3"
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 relative question-input-container">
                      <input
                        ...
                        onFocus={() =>
                          handleQuestionChange(index, 'showSuggestions', true)
                        }
                      ...
                      {question.showSuggestions && (
                        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
                          ...
                          {getCategorizedSuggestions(question.question)
                            .length === 0 &&
                            getFilteredSuggestions(question.question).length ===
                              0 &&
                            question.question.trim() && (
