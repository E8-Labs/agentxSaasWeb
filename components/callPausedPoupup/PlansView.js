...
function PlansView({
    ...
    useEffect(() => {
        if (selectedPlan && getCurrentPlans().length > 0) {
            const matchingPlan = getCurrentPlans().find(plan => 
        ...
        <div>
            ...
            <div className="flex flex-col items-center gap-4 w-full z-10 relative mt-5">
                ...
                <div className="text-center text-[14px] font-[400] text-black max-w-xl">
                    {`We noticed you've renewed your current plan`} <span className="font-semibold">twice</span> in the last <span className="font-semibold">24 hours</span>.{` To ensure you get the best value, we've temporarily paused your calls.`}
                </div>
            </div>
