...
const LogoCropper = ({ open, onClose, imageSrc, onCropComplete, aspectRatio = 3 }) => {
  ...
  // Debug logging
  useEffect(() => {
    console.log('🖼️ [LogoCropper] Props changed:', { open, hasImageSrc: !!imageSrc });
  }, [open, imageSrc]);
  ...
  const handleCropComplete = async () => {
    ...
    try {
      ...
      if (croppedImage.file.size > 524288) {
        setError("Cropped image is still too large. Please try a different crop area.");
        setLoading(false);
        return;
      }
  ...
  console.log('🖼️ [LogoCropper] Rendering with open:', open, 'imageSrc:', !!imageSrc);
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            <div className="text-sm text-gray-600">
              Recommended: 600 × 200 px upload. Final display: Max 120px width × 32px height (3:1 aspect ratio)
            </div>

            <div className="relative" style={{ width: "100%", height: "400px", background: "#333" }}>
              {imageSrc ? (
                <Cropper
                  image={imageSrc}
                  crop={crop}
