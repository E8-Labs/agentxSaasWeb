...
const SolarRepAgentSignUp = ({ handleContinue, handleWaitList, handleSolarAgentBack, length = 6, onComplete }) => {
    ...
    useEffect(() => {
        if (userName && userEmail && userPhoneNumber &&
            customerService && companyName && installationVolume &&
            projectSize && ClientType &&
            emailCheckResponse?.status === true && checkPhoneResponse?.status === true) {
            setShouldContinue(false);
        } else if (!userName || !userEmail || !userPhoneNumber ||
            !userFarm || !userBrokage || !userTransaction || customerService || companyName || installationVolume || projectSize || ClientType ||
            userTransaction || checkPhoneResponse?.status === false || emailCheckResponse?.status === false) {
            setShouldContinue(true);
        }
    }, [userName, userEmail, userPhoneNumber, checkPhoneResponse, emailCheckResponse, customerService, companyName, installationVolume, projectSize, ClientType,]);
        ...
        <div style={{ width: "100%" }} className="overflow-y-hidden flex flex-row justify-center items-center">
            <div className='bg-white rounded-2xl mx-2 w-full md:w-10/12 max-h-[90%] py-4 overflow-auto scrollbar scrollbar-track-transparent scrollbar-thin scrollbar-thumb-purple'>
                <div className='h-[80vh]'>
                    ...
                    <div className='flex flex-col items-center px-4 w-full h-[90%]'>
                        <div className='mt-6 w-11/12 md:text-4xl text-lg font-[600]' style={{ textAlign: "center" }} onClick={handleContinue}>
                            Your Contact Information for solarAgent
                        </div>
                ...
                <div className='h-[10%]'>
                    ...
                    <Footer handleContinue={handleVerifyPopup} handleBack={handleSolarAgentBack} registerLoader={registerLoader} shouldContinue={shouldContinue} />
                </div>
