...
const SalesDevAgent = ({ handleContinue, handleSalesAgentBack, length = 6, onComplete, handleWaitList }) => {
    ...
    useEffect(() => {
        if (userName && userEmail && userPhoneNumber && customerService && company &&
            emailCheckResponse?.status === true && checkPhoneResponse?.status === true) {
            setShouldContinue(false);
        } else if (!userName || !userEmail || !userPhoneNumber ||
            !customerService || !company || !userTransaction ||
            checkPhoneResponse?.status === false || emailCheckResponse?.status === false) {
            setShouldContinue(true);
        }
    }, [userName, userEmail, userPhoneNumber, customerService, company, userTransaction, checkPhoneResponse, emailCheckResponse]);
        ...
        <div style={{ width: "100%" }} className="overflow-y-hidden flex flex-row justify-center items-center">
            <div className='bg-white rounded-2xl mx-2 w-full md:w-10/12 max-h-[90%] py-4 overflow-auto scrollbar scrollbar-track-transparent scrollbar-thin scrollbar-thumb-purple'>
                ...
                <div className='h-[10%]'>
                    ...
                    <Footer handleContinue={handleVerifyPopup} handleBack={handleSalesAgentBack} registerLoader={registerLoader} shouldContinue={shouldContinue} />
                </div>
