...
function AdminBasicInfo({ selectedUser }) {
  ...
  useEffect(() => {
    const getData = async () => {
      const LocalData = await AdminGetProfileDetails(selectedUser)
      if (LocalData) {
  ...
  const getProfile = async () => {
    try {
      await AdminGetProfileDetails(selectedUser);
    } catch (error) {
  ...
  return (
    <div
      className="w-full h-[40vh] flex flex-col items-start px-8 py-2"
      style={{
        paddingBottom: "50px",
        height: "100%",
        overflow: "auto",
