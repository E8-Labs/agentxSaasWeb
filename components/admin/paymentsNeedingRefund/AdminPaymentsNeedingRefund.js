...
const AdminPaymentsNeedingRefund = () => {
  ...
  const markRefundAsProcessed = async (paymentId, refundId, notes) => {
    try {
      ...
      const response = await axios.post(Apis.markRefundProcessed, {
        paymentId,
        refundId,
        notes
      }, {
      ...
      if (response.data.status) {
        ...
        throw new Error(response.data.message || 'Failed to mark refund as processed');
      }
    } catch (err) {
  ...
  const handleMarkAsProcessed = (payment) => {
    const refundId = prompt(`Enter the Stripe refund ID for ${payment.user.name}:`);
    if (refundId) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto h-full overflow-y-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Payments Needing Refund</h1>
        <p className="text-gray-600">Monitor and process payment refunds that need manual handling</p>
      </div>

      {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-lg font-semibold text-gray-700">Pending Refunds</h3>
          <p className="text-2xl font-bold text-red-600">{summary.pending_refunds.count}</p>
          <p className="text-sm text-gray-500">Total: {formatAmount(summary.pending_refunds.total_amount)}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-lg font-semibold text-gray-700">Average Refund</h3>
          <p className="text-2xl font-bold text-purple-600">{formatAmount(summary.pending_refunds.average_amount)}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-lg font-semibold text-gray-700">Processed Refunds</h3>
          <p className="text-2xl font-bold text-green-600">{summary.processed_refunds.count}</p>
          <p className="text-sm text-gray-500">Total: {formatAmount(summary.processed_refunds.total_amount)}</p>
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Charge Date</th>
                  ...
                  <tr key={payment.id || index} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-xs text-gray-400">{payment.user.company}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-xs text-gray-400">{payment.environment}</div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getUrgencyColor(payment.urgency)}`}>
                        {payment.urgency || 'normal'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900 max-w-xs truncate" title={payment.refundReason}>
                        {payment.refundReason}
                      </div>
                      <div className="text-xs text-gray-500">{payment.processingStatus}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {payment.refundId ? (
                        ...
                        >
                          {processingRefund === payment.id ? 'Processing...' : 'Mark as Processed'}
                        </button>
                      )}
                    </td>
                  </tr>
      ...
      {payments.length === filters.limit && (
        <div className="mt-6 flex justify-between items-center">
          <div className="text-sm text-gray-700">
            Showing {filters.offset + 1} to {filters.offset + payments.length} results
          </div>
