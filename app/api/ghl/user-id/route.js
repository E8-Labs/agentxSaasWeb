...
export async function GET() {
    ...
    if (!token || !locationId) {
        return NextResponse.json(
            { error: "Not authorized", hasToken: !!token, hasLocationId: !!locationId },
            { status: 401 }
        );
    }
    ...
    for (const url of candidates) {
        ...
        if (r.status === 403) {
            ...
            return NextResponse.json({ error: "Missing users.readonly on token", url, body: text }, { status: 403 });
        }
        ...
        if (r.ok) {
            ...
            if (Array.isArray(users) && users.length) {
                ...
                const res = NextResponse.json({ userId, from: url, count: users.length });
            ...
            return NextResponse.json({ upstreamStatus: r.status, url, body: text }, { status: r.status });
        }
    }
    ...
    let cal; try { cal = JSON.parse(calText); } catch { }
