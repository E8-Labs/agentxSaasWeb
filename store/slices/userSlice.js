...
export const selectIsAgencyTeamMember = (state) =>
  state.user.user?.agencyTeamMember
export const selectUserLoading = (state) => state.user.loading
export const selectUserError = (state) => state.user.error

// Plan-specific selectors
export const selectUserPlan = (state) => state.user.user?.plan
export const selectPlanCapabilities = (state) =>
  state.user.user?.planCapabilities
...
export const selectMaxAgents = (state) =>
  state.user.user?.planCapabilities?.maxAgents || 0
// Note: currentUsage.maxAgents contains the current count, not a limit
export const selectCurrentAgents = (state) =>
  state.user.user?.currentUsage?.maxAgents || 0
export const selectAllowVoicemail = (state) =>
  state.user.user?.planCapabilities?.allowVoicemail || false
export const selectAllowToolsAndActions = (state) =>
  state.user.user?.planCapabilities?.allowToolsAndActions || false
export const selectAllowKnowledgeBases = (state) =>
  state.user.user?.planCapabilities?.allowKnowledgeBases || false
